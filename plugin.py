...
class OngekiGachaPlugin(GrowthCommandsMixin, TaskCommandsMixin, MaiBotPlugin):
    ...
    def __init__(self) -> None:
        ...
        self._card_images_ready = False
        self._ui_assets_ready = False
    ...
    async def on_config_update(self, scope: str, config_data: dict[str, object], version: str) -> None:
        ...
        async with self._lock:
            ...
            self._card_images_ready = self._card_data_quick_ready(cards_dir, card_info_path)
        ...
        self._run_daily_maintenance()
        if self.config.task.enabled:
            self._task_reset_task = asyncio.create_task(self._daily_task_reset_loop())
    ...
    def _rules_text(self) -> list[str]:
        ...
        return [
            ...
            f"/陪伴 每个账号每天一次，按 {self._timezone_label(self.config.economy.tz_offset_hours)} 日期结算",
            ...
            "发放与接收记录都按用户写入审计日志",
            "QQ 官方机器人需先 /绑定QQ <QQ号>；该平台的 /奖励 与 /终极完成请用数字 QQ 指定目标",
            "【随机任务】",
            ...
            "【说明】",
            f"签到、任务、周保底与轮替均按 {self._timezone_label(self.config.economy.tz_offset_hours)} 计算",
    ...
    def _timezone_label(offset_hours: int) -> str:
        ...
        return "UTC" if offset == 0 else f"UTC{offset:+d}"
    ...
    def _is_admin(self, user_id: str) -> bool:
        ...
        if user_id == "local-operator" and self.config.admin.allow_local_operator:
            return True
        canonical = self._resolve_identity(user_id) or user_id
        raw_ids = self.config.admin.admin_ids or []
        for item in raw_ids:
            raw = str(item).strip()
            if not raw:
                continue
            if raw == user_id or raw == canonical:
                return True
            if self._db is not None:
                resolved = self._db.resolve_identity(raw)
                if resolved is not None and resolved == canonical:
                    return True
            if self._db is not None and raw == self._db.resolve_account(raw):
                if raw == canonical:
                    return True
        return False

    def _resolve_identity(self, alias: str) -> str | None:
        """把外部别名解析到规范账号 ID；未绑定的 openid 直接作为账号 ID。"""
        alias = str(alias or "").strip()
        if not alias:
            return None
        if self._db is not None:
            try:
                resolved = self._db.resolve_account(alias)
            except Exception:
                resolved = alias
            if resolved:
                return resolved
    ...
    @staticmethod
    def _extract_at_target_ids(kwargs: dict[str, Any]) -> list[str]:
        """提取所有被 @ 的目标 ID（支持 openid 等非数字 ID）。"""
    ...
    def _extract_at_target_id(kwargs: dict[str, Any]) -> str | None:
        targets = OngekiGachaPlugin._extract_at_target_ids(kwargs)
        return targets[0] if targets else None

    def _mention_grant(self, kwargs: dict[str, Any]) -> tuple[str, int, str] | None:
        """正文里没有 @ 目标时，用消息组件中的唯一 @ 目标兜底解析。

        QQ 官方机器人群消息会把被 @ 的成员从正文里删掉（只留下空白），
        目标只存在于消息组件的 mention 列表中，因此不能依赖正文。
        """
        text = str(kwargs.get("text") or "")
        match = re.match(r"^/奖励\s*(?:<@!?[^\s>]+>|@\S+)?\s*(\d+)", text)
        if match is None:
        ...
        if amount <= 0:
            return None
        return self._at_component_grant(kwargs, amount, "")

    def _at_component_grant(
        self,
        kwargs: dict[str, Any],
        amount: int,
        note: str,
    ) -> tuple[str, int, str] | None:
        """用消息组件里唯一的 @ 目标补全奖励目标；多于一个则不猜。"""
        targets = OngekiGachaPlugin._extract_at_target_ids(kwargs)
        if len(targets) != 1:
            return None
        target = targets[0]
        return (self._resolve_identity(target) or target), amount, note
    ...
    def _parse_grant(self, kwargs: dict[str, Any]) -> tuple[str, int, str] | None:
            ...
            if raw_amount.isdigit() and int(raw_amount) > 0:
                ...
                if target_id.isdigit():
                    ...
                    if resolved_id is None:
                        return None
                    return resolved_id, amount, note
                ...
                if target_at.startswith("<"):
                    ...
                    if resolved_id:
                        return resolved_id, amount, note
                if not target_id and not target_at:
                    # 目标来自 @ 消息组件：正文里没有 @ 占位符（QQ 官方群消息）
                    resolved = self._at_component_grant(kwargs, amount, note)
        ...
        if match is None:
            # 文本里没有 @ 占位符（部分平台把 @ 作为独立组件）时，用组件兜底。
            return self._mention_grant(kwargs)
    ...
    async def _daily_task_reset_loop(self) -> None:
        ...
        try:
            while True:
                self._run_daily_maintenance()
                await asyncio.sleep(self._seconds_until_next_reset(
    ...
    @staticmethod
    def _seconds_until_next_reset(offset_hours: int, *, now_utc: datetime | None = None) -> float:
        """距离配置时区下一个零点的秒数。"""
    ...
    def _user_id(self, kwargs: dict[str, Any]) -> str:
        """返回命令使用的规范账号 ID（跨机器人账号共享同一 QQ 的数据）。"""
        user_id = str(kwargs.get("user_id") or kwargs.get("sender_id") or "").strip()
        if not user_id:
            if kwargs.get("is_local_operator"):
                return "local-operator"
            return "unknown"
        if user_id in {"local-operator", "unknown"}:
            return user_id
        if self._db is not None:
            try:
                return self._db.resolve_account(user_id)
            except Exception:
                return user_id
        return user_id
        ...
        async with self._lock:
            ...
            if player.savings_bonus_start_date:
                try:
                    ...
                    pass
        lines = [
            f"点数 {player.points}｜花之碎片 {fragments}",
    ...
    @Command(
        ...
        pattern=(
            # 目标可以省略：QQ 官方机器人把被 @ 的成员放在消息组件里
            # （正文被清空），此时用组件兜底解析。
            r"^/奖励\s+"
            r"(?:(?:(?P<target_at>@\S+|@?<\S+>)|(?P<target_id>\d+))\s+)?"
            r"(?P<amount>\d+)(?:\s+(?P<note>.+))?\s*$"
        ),
    )
    async def handle_grant_points(
        ...
        **kwargs: dict[str, Any],
        ...
        if parsed is None:
            logger.info(
                "奖励指令未解析出目标：text=%r at_targets=%r",
                str(kwargs.get("text") or ""),
                self._extract_at_target_ids(kwargs),
            )
            text = (
                "没识别到目标｜用法 /奖励 @用户 <点数> 或 /奖励 <QQ号> <点数>"
            )
            if not str(user_id).isdigit():
                # QQ 官方机器人只下发 openid，@ 普通成员不会带进消息里
                text += "\nQQ 官方拿不到 @ 的 openid，请用数字 QQ（对方先 /绑定QQ）"
            await self._send_text(stream_id, text)
            return True, text, True
    ...
    async def handle_bind_identity(
        ...
        **kwargs: dict[str, Any],
        ...
        if not raw_qq.isdigit():
            text = "用法 /绑定QQ <QQ号>，例 /绑定QQ 123456789"
            await self._send_text(stream_id, text)
            return True, text, True
        if self._db is None:
            text = "插件未就绪，请稍后重试"
            ...
            if action == "列表":
                ...
                for entry in active_entries:
                    ...
                    if max_points <= 0:
                        ...
                        continue
                    lines.append(
                        f"天井 {state.select_points}/{max_points}"
                        f"（上限 {max_points}）｜可选 {len(selectable)} 张"
                    )
                ...
                def exchange_text(pool: PoolEntry, max_points: int) -> str:
                    ...
                    return (
                        ...
                        f"{self._card_display_name(card, 24)}（ID {card.id}）\n"
                        f"持有 {receipt.copies} 张｜花之碎片 +{receipt.fragments}"
                    )
