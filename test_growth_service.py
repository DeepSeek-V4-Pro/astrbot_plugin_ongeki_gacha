...
class GrowthServiceTests(unittest.TestCase):
    ...
    def test_daily_global_and_configured_timezone(self):
        self.own()
        self.own(self.other)
        self.service.partner("u", 1000, "a")
        self.service.tz_offset_hours = 8
        with patch.object(self.db, "current_date_str", return_value="2026-09-12") as date_clock:
            ...
            self.assertTrue(all(call.args == (8,) for call in date_clock.call_args_list))
        with patch.object(self.db, "current_date_str", return_value="2026-09-13") as date_clock:
            self.assertTrue(self.service.accompany("u", "e")["success"])
            date_clock.assert_called_with(8)
