...
class GrowthService:
    ...
    def accompany(self, qq_id: str, request_id: str) -> dict:
        def execute(conn, now):
            today = self.db.current_date_str(self.tz_offset_hours)
            latest = conn.execute(
                "SELECT MAX(utc_date) FROM growth_daily_usage "
                "WHERE qq_id=? AND action='companion'", (qq_id,)
            ).fetchone()[0]
            if latest and latest > today:
                raise GrowthError("陪伴记录晚于当前日期，请检查服务器时间或时区配置")
    ...
    def buy_gift(self, qq_id: str, size: str, quantity: int, request_id: str) -> dict:
        ...
        def execute(conn, now):
            ...
            week = self._week_key(self.db.current_date_str(self.tz_offset_hours))
            action = f"gift_purchase_{size}"
            latest = conn.execute(
                "SELECT MAX(utc_date) FROM growth_daily_usage WHERE qq_id=? AND action=?",
                (qq_id, action),
            ).fetchone()[0]
            if latest and latest > week:
                raise GrowthError("购买记录晚于当前周，请检查服务器时间或时区配置")
