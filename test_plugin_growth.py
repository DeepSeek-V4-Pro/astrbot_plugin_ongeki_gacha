...
class PluginGrowthTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hot_reload_preserves_voice_limiter_and_unload_releases(self):
        voice=self.plugin._voice;voice._attempts['user'].append(123)
        await self.plugin.on_config_update('self',{},'test')
        self.assertIs(self.plugin._voice,voice)
        self.assertEqual(list(voice._attempts['user']),[123])
    ...
    async def test_affection_entry_and_decorated_affection(self):
        ...
        self.assertFalse(snap['inventory'])
        self.sender.image.reset_mock()
        response=await self.dispatch('/好感 柏木美亜','affection')
        snap=self.plugin._growth.snapshot('user')
        self.assertEqual(len(snap['inventory']), 1)
        self.assertIn(snap['inventory'][0]['card_id'], STARTER_CARD_IDS)
    ...
    async def test_bad_catalog_load_cleans_up_database(self):
        await self.plugin.on_unload()
        with patch('astrbot_plugin_ongeki_gacha.plugin.GrowthCatalog',side_effect=ValueError('bad catalog')):
    ...
    async def test_advanced_review_grants_bloom_ticket(self):
        task = self.plugin._db.create_task(
            'user', task_kind='advanced', game='ongeki', song_id='advanced-1',
            song_title='Advanced', target_level_value=13.5,
            advanced_limit=self.plugin.config.task.advanced_count,
        )
        self.assertTrue(task.success)
        self.assertTrue(self.plugin._db.submit_task(task.task_id, 'user').success)
        reply = await self.dispatch(f'/任务审核 {task.task_id} SSS', 'advanced-review')
        self.assertIn('解花券 +1', reply[1])
        items = {
            row['item_id']: row['quantity']
            for row in self.plugin._growth.snapshot('user')['player_items']
        }
    ...
    async def test_bloom_command_sends_result_image(self):
        ...
        try:
            change_item(conn,'user','bloom_ticket',1)
            conn.execute(
    ...
    async def test_catalog_render_failure_falls_back_to_text(self):
        with patch('astrbot_plugin_ongeki_gacha.plugin.render_catalog_index',side_effect=RuntimeError('boom')):
