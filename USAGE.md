...
音击抽卡模拟器是一个 AstrBot 插件，用于在聊天环境中模拟音击卡牌抽卡、收藏和成长体验。
...
2. 在 AstrBot WebUI「插件管理」中加载或重载插件。
...
可通过 AstrBot 配置面板中的「素材路径」修改 `cards_dir` 和 `card_info_json` 为外部绝对路径。
...
| `/概率` | 查看当前模拟权重、消耗和保底规则 |
| `/绑定QQ <QQ号>` | 把当前 AstrBot 内部 ID 绑定到数字 QQ，合并重复档案 |
...
- 在 AstrBot WebUI 的「管理员」分组中，把管理员 ID 加入 `admin_ids` 列表。
- 管理员可使用 `/奖励 @用户 <点数> [备注]` 或 `/奖励 <QQ号> <点数> [备注]` 为指定用户增加点数。
- QQ 官方适配器下发送者 ID 是 AstrBot/QQ 官方 openid；请用 `/绑定QQ <QQ号>` 绑定后，
...
默认配置由 `_conf_schema.json` 生成，所有数值均可在 AstrBot WebUI 的插件配置面板中调整。
下面列出与默认值等价的配置内容，仅作对照；AstrBot 运行时不需要 `config.toml`。
