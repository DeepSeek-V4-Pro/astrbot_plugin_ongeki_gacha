...
当前版本：1.3.0（按上游 1.3.1 内容适配，2026-09-23）
...
音击抽卡模拟器是一个 AstrBot 插件，在聊天环境里模拟音击（オンゲキ）的抽卡、卡牌收藏、
...
1. 把 `astrbot_plugin_ongeki_gacha` 目录放进 `<ASTRBOT_ROOT>/data/plugins/`，在 WebUI 插件管理中加载或重载。
2. 安装时 AstrBot 会从 `requirements.txt` 安装 Pillow 与 Pydantic；默认读取插件目录下的 `assets/card_data/`。
3. 配置由 `_conf_schema.json` 生成，保存在 `<ASTRBOT_ROOT>/data/config/astrbot_plugin_ongeki_gacha_config.json`。
4. 数据库保存在 `<ASTRBOT_ROOT>/data/plugin_data/astrbot_plugin_ongeki_gacha/ongeki_gacha.db`。升级前备份数据库与配置文件。

支持 OneBot（aiocqhttp）与 QQ 官方机器人（qq_official）。QQ 官方平台先使用 `/绑定QQ <QQ号>` 绑定数字 QQ；不同机器人账号需要各自绑定同一 QQ 才能共享资料。该平台不提供普通成员的 @ 身份，`/奖励` 与 `/终极完成` 请用数字 QQ 指定目标。
...
替换插件文件后，在 AstrBot WebUI 重新加载插件或重启实例。

### 2.2 从 1.2.1 升级

1. 备份 `data/plugin_data/astrbot_plugin_ongeki_gacha/ongeki_gacha.db` 和 `data/config/astrbot_plugin_ongeki_gacha_config.json`。
2. 替换插件代码和随包 JSON 索引，保留本地补入的图片、字体与语音素材。
3. 在 WebUI 重载插件；首次启动会先生成迁移备份，再就地迁移数据库。旧配置缺少的新字段由框架补默认值，已有管理员、卡池和经济配置保留。
4. 依次检查 `/帮助`、`/点数`、`/卡池`、`/好感 列表`、`/任务列表`。缺少图片素材时文字回退属于预期行为。

旧好感曲线数据会一次性迁移：好感点按新版曲线换算并保持等级；旧解花阶段、库存和终极任务记录尽量保留。待审核任务按审核时的配置发奖。部署时若设 `economy.tz_offset_hours = 8`，每日与每周额度按北京时间切换。
...
| `/绑定QQ <QQ号>` | QQ 官方平台绑定数字 QQ，合并旧数字 QQ 档案 |
| `/规则` | 以图片卡片查看完整规则 |
| `/帮助` | 精简命令列表 |

### 3.2 管理员命令

管理员需要在 WebUI 的 `admin.admin_ids` 中配置 QQ 号或已绑定身份。
...
- 保底：11 连必得 SR 或以上；5 连每用户每个配置时区周首次触发一次 SR 或以上保底。
...
- 普通 / 挑战 / 高级挑战未完成任务在配置时区的每日 00:00自动过期；待审核任务保留。
...
- `/伙伴 <角色姓名>` 选择伙伴；`/陪伴` 每个账号每天一次，+300 好感，按配置时区日期结算，换群不重置。
...
- 签到、任务、陪伴、礼物额度、周保底与卡池轮替均按 `economy.tz_offset_hours` 计算；默认 UTC，设为 8 即北京时间。
...
python -m astrbot_plugin_ongeki_gacha.tools.verify_growth_install       # 养成素材与语音逐项散列
python -m astrbot_plugin_ongeki_gacha.tools.verify_text_fallback        # 无素材兜底
```

## 10. 配置项

AstrBot 从 `_conf_schema.json` 生成 WebUI 配置面板与默认值，并将配置写入 `data/config/astrbot_plugin_ongeki_gacha_config.json`。代码中的 `config_model.py` 负责校验。升级时框架会补入新字段，已有值保留；自由键值的 `growth.rules_overrides` 使用 AstrBot 的 `dict` 类型。

| 分组 | 常用设置与默认值 |
| --- | --- |
| `assets` | `cards_dir = assets/card_data`、`card_info_json = assets/card_data/card_info_merged.json`，可填绝对路径 |
| `pool` | R/SR/SSR 权重 77/20/3，UP 倍率 10，15 天轮替；`rotation_mode` 可选 `cycle` / `official` |
| `economy` | 单抽/五连/十一连 50/250/500；签到 70～110；`tz_offset_hours = 0`，中国大陆部署建议设为 8 |
| `monthly_card` | 1000 点 / 30 天；每日 +80 点；赠送两次半价五连 |
| `task` | 普通/挑战/高级挑战每日 3/2/1 次；任务历史保留 30 天；曲库 URL 和奖励可调整 |
| `growth` | 17 名主角色养成、语音、礼物、解花和奖励；`voice_enabled` 控制语音，`rules_overrides` 可覆盖规则表 |
| `admin` | `admin_ids` 配置数字 QQ 或已绑定身份；`allow_local_operator` 控制本地管理身份 |
| `ui` | `render_cache_ttl_seconds` 默认 86400，其他图片缓存与短回复阈值可调整 |

`voice_enabled = false` 只停用语音，不影响养成；`automatic_voice_enabled` 仅控制自动回应。`strict_pool_cards = true` 时按池子候选卡抽取。修改时区会影响接下来的日、周边界；应保持服务器时间准确。
...
- **语音点播提示限流**：默认 10 秒冷却、每分钟 5 次，可在 WebUI 的 `growth` 分组调整。
